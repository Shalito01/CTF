...
Okay, you can see that the NX option is enabled as `checksec` said. And exploit is broken. Let's try without the script:
```bash
...
But, it doesn't mean that you can't exploit it now. You can apply two exploitation techniques here: [[Ret2libc|Return to libc attack(ret2libc)]] and [[ROP|Return-Oriented programming(ROP)]].
